...
def create_video_settings_tab(self, parent):
    ...
    # Video Codec Section
    codec_frame = ttk.LabelFrame(scrollable_frame, text="Video Codec", padding=15)
    codec_frame.pack(fill="x", padx=10, pady=10)

    ttk.Label(codec_frame, text="Codec:").pack(anchor="w")
    ...
    codec_combo.pack(fill="x", pady=(5, 10))

    # Quality Settings
    quality_frame = ttk.LabelFrame(
        scrollable_frame, text="Quality Settings", padding=15
    )
    quality_frame.pack(fill="x", padx=10, pady=10)

    ttk.Label(quality_frame, text="CRF (Lower = Better Quality):").pack(anchor="w")
    crf_frame = ttk.Frame(quality_frame)
    crf_frame.pack(fill="x", pady=(5, 10))

    crf_scale = ttk.Scale(
        crf_frame, from_=0, to=51, variable=self.crf_var, orient="horizontal"
    )
    crf_scale.pack(side="left", fill="x", expand=True)

    self.crf_label = ttk.Label(crf_frame, text="23")
    self.crf_label.pack(side="right", padx=(10, 0))

    crf_scale.configure(
        command=lambda v: self.crf_label.configure(text=str(int(float(v))))
    )

    ttk.Label(quality_frame, text="Preset (Speed vs Compression):").pack(anchor="w")
    ...
    ttk.Label(quality_frame, text="Video Bitrate:").pack(anchor="w")
    ...
    bitrate_combo.pack(fill="x", pady=(5, 0))

    # Resolution Settings
    resolution_frame = ttk.LabelFrame(scrollable_frame, text="Resolution", padding=15)
    resolution_frame.pack(fill="x", padx=10, pady=10)

    ttk.Label(resolution_frame, text="Target Resolution:").pack(anchor="w")
    ...
    resolution_combo.pack(fill="x", pady=(5, 10))

    ttk.Label(resolution_frame, text="Frame Rate:").pack(anchor="w")
...
def create_audio_settings_tab(self, parent):
    """Create audio encoding settings tab."""
    # Create scrollable frame
    canvas = ttk.Canvas(parent)
    scrollbar = ttk.Scrollbar(parent, orient="vertical", command=canvas.yview)
    scrollable_frame = ttk.Frame(canvas)
    ...
    # Audio Codec Section
    codec_frame = ttk.LabelFrame(scrollable_frame, text="Audio Codec", padding=15)
    codec_frame.pack(fill="x", padx=10, pady=10)
    ...
    audio_codec_combo.pack(fill="x", pady=(5, 0))
    ...
    ttk.Label(quality_frame, text="Audio Bitrate:").pack(anchor="w")
    ...
    audio_bitrate_combo.pack(fill="x", pady=(5, 10))

    ttk.Label(quality_frame, text="Sample Rate:").pack(anchor="w")
    ...
    sample_rate_combo.pack(fill="x", pady=(5, 10))

    ttk.Label(quality_frame, text="Channels:").pack(anchor="w")
    ...
    channels_combo.pack(fill="x", pady=(5, 0))
